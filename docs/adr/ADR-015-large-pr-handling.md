...
1. **Coverage-priority selection (default)**: when the filtered diff exceeds the threshold, split it into per-file sections (grouped by path so repeated `diff --git` headers for one file are counted once), sort by churn (size, descending), and greedily pack sections into the threshold-sized budget. Files that don't fit are listed as NOT reviewed. If a single file exceeds the budget alone, its first `budget` chars are included and marked partially reviewed. A large diff with no parseable sections keeps the truncation note and still instructs Jules to state that coverage is limited.
